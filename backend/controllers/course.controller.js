...
export const createCourse = async (req, res) => {
  ...
  try {
    ...
    const { image } = req.files;                //checks for uploaded image
    if (!req.files || Object.keys(req.files).length === 0) {
      return res.status(400).json({ errors: "No file uploaded" });
    }

    const allowedFormat = ["image/png", "image/jpeg"];//check for uploaded image
    if (!allowedFormat.includes(image.mimetype)) {    //type of image
      return res
        .status(400)
        .json({ errors: "Invalid file format. Only PNG and JPG are allowed" });
    }

    // claudinary code-cloud service for uploading, transforming, and delivering media files.
